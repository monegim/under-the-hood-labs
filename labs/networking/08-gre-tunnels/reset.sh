#!/usr/bin/env bash
# Lab 8 (GRE Tunnels) — tears down and redeploys the topology, then
# re-applies the README's Steps 1-5 live config (addressing, forwarding,
# tunnel creation, routing) — none of this is baked into the topology file.
set -uo pipefail
...
echo "[reset] enabling forwarding on the tunnel endpoints..."
...
echo "[reset] routing the overlay subnets through the tunnel..."
...
if docker exec "$HOSTA" ping -c 2 -W 2 10.2.2.10 >/dev/null 2>&1; then
  echo "[reset] hostA -> hostB reachable, tunnel is healthy"
else
