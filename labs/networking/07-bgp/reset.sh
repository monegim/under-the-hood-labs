#!/usr/bin/env bash
# Lab 7 (BGP) — tears down and redeploys the topology, then re-applies the
# addressing + eBGP config from the README's Steps 2-3 (this config is
# applied live via vtysh, not baked into the topology file — only the
# /etc/frr/daemons bind mount, which turns bgpd on, is baked in).
set -uo pipefail
...
echo "[reset] addressing interfaces (README Step 2)..."
...
echo "[reset] configuring eBGP peering (README Step 3)..."
docker exec "$R1" vtysh \
  -c "configure terminal" \
  -c "router bgp 65001" \
