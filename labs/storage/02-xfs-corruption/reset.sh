#!/usr/bin/env bash
# Lab 2 reset — unmount, detach the loop device, and blow away the
# backing file entirely (setup.sh is not safe to re-run on top of an
# existing mount/loop device), then re-run setup.sh to recreate the
# corruption incident.
set -uo pipefail
...
echo "[reset] re-running setup.sh to recreate the XFS corruption incident..."
